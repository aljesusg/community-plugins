...
import { PFBadges, PFBadgeType, PFColors } from '../Pf';
import { getHomeCluster, IconType, icons } from '../../config';
import {
...
const gatewayIconStyle = style({
  color: PFColors.White,
  display: 'flex',
  marginTop: '-0.125rem'
});

const waypointIconStyle = style({
  color: PFColors.White,
  display: 'flex',
  marginLeft: '0.125rem',
  marginTop: '-0.125rem'
...
export const setNodeLabel = (
  ...
  const homeCluster = getHomeCluster(config)?.name || CLUSTER_DEFAULT;

...
export const decorateGraphData = (graphData: GraphElements, duration: number, config: ComputedServerConfig): DecoratedGraphElements => {
  ...
  if (graphData) {
    if (graphData.nodes) {
      decoratedGraph.nodes = graphData.nodes.map((node: GraphNodeWrapper) => {
        ...
        const isIstio = config.istioNamespace === decoratedNode.data.namespace ? true : undefined;
        // prettier-ignore

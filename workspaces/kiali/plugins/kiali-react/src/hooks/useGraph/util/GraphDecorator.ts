import { GraphElements, GraphNodeWrapper, GraphEdgeWrapper, hasProtocolTraffic } from '@backstage-community/plugin-kiali-common';
import {
...
import { isIstioNamespace } from '../../../types/ServerConfig';
  ...
  if (graphData) {
    if (graphData.nodes) {
      decoratedGraph.nodes = graphData.nodes.map((node: GraphNodeWrapper) => {
        ...
        const isIstio = isIstioNamespace(decoratedNode.data.namespace, istioNs)
          ? true
          : undefined;
        // prettier-ignore

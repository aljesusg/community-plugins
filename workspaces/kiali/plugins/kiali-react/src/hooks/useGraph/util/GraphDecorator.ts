import { GraphElements, GraphNodeWrapper, GraphEdgeWrapper, hasProtocolTraffic, ComputedServerConfig } from '@backstage-community/plugin-kiali-common';
import {
  ...
  if (graphData) {
    if (graphData.nodes) {
      decoratedGraph.nodes = graphData.nodes.map((node: GraphNodeWrapper) => {
        ...
        const isIstio = config.istioNamespace === decoratedNode.data.namespace
          ? true
          : undefined;
        // prettier-ignore

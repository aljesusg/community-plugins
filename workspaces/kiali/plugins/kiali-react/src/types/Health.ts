...
import { ComputedServerConfig, defaultServerConfig, HealthAnnotationType, ToleranceConfig } from '@backstage-community/plugin-kiali-common';
...
export class ServiceHealth extends Health {
  public static fromJson = (ns: string, srv: string, json: any, ctx: HealthContext, config: ComputedServerConfig) =>
    new ServiceHealth(ns, srv, json.requests, ctx, config);

  private static computeItems(ns: string, srv: string, requests: RequestHealth, ctx: HealthContext, config: ComputedServerConfig): HealthConfig {
    const items: HealthItem[] = [];
    let statusConfig: HealthItemConfig | undefined = undefined;
    if (ctx.hasSidecar) {
      // Request errors
      const reqError = calculateErrorRate(ns, srv, 'service', requests, config);
      const reqErrorsText =
        reqError.errorRatio.global.status.status === NA
          ? 'No requests'
          : reqError.errorRatio.global.status.value.toFixed(2) + '%';
      const item: HealthItem = {
        title: createTrafficTitle(getName(ctx.rateInterval, config).toLocaleLowerCase('en-US')),
  ...
  constructor(ns: string, srv: string, public requests: RequestHealth, ctx: HealthContext, config: ComputedServerConfig) {
    super(ServiceHealth.computeItems(ns, srv, requests, ctx, config));
  }
}

export class AppHealth extends Health {
  public static fromJson = (ns: string, app: string, json: any, ctx: HealthContext, config: ComputedServerConfig) =>
    new AppHealth(ns, app, json.workloadStatuses, json.requests, ctx, config);
  ...
  private static computeItems(
    ...
    // Request errors
    if (ctx.hasSidecar) {
      const reqError = calculateErrorRate(ns, app, 'app', requests, config);
  ...
  constructor(
    ...
    super(AppHealth.computeItems(ns, app, workloadStatuses, requests, ctx, config));
  }
}

export class WorkloadHealth extends Health {
  public static fromJson = (ns: string, workload: string, json: any, ctx: HealthContext, config: ComputedServerConfig) =>
    new WorkloadHealth(ns, workload, json.workloadStatus, json.requests, ctx, config);
  ...
  private static computeItems(
    ...
    // Request errors
    if (ctx.hasSidecar) {
      const reqError = calculateErrorRate(ns, workload, 'workload', requests, config);
      const reqIn = reqError.errorRatio.inbound.status;
      const reqOut = reqError.errorRatio.outbound.status;
      const both = mergeStatus(reqIn.status, reqOut.status);
      const item: HealthItem = {
  ...
  constructor(
    ...
    super(WorkloadHealth.computeItems(ns, workload, workloadStatus, requests, ctx, config));
  }
}
